      ...
      (if (≠ N₁ N₂)
        (let [L₁₂ (math.sqrt (* N₁ N₂))
          ...
          (values {} (define-circuit
            :consumer (.. id "-prim")  A T (inductance L₁)
            :consumer (.. id "-sec")   T C (inductance L₂)
            :consumer (.. id "-T")     T B (inductance L₁₂)
            :consumer (.. id "-resis") B D (real transformer-resis))))
        (values {} (define-circuit
          :consumer (.. id "-fst") A C (real transformer-resis)
          :consumer (.. id "-snd") B D (real transformer-resis))))))))